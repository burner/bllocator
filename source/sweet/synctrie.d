...
import std.array : empty, front, back;
...
class TrieThread : Thread {
	...
	size_t i;
	size_t n;
	shared(Trie!(string,size_t)) trie;

	this(string[] str, size_t i, size_t n, shared(Trie!(string,size_t)) trie) {
		super(&run);
		this.str = str;
		this.i = i;
		this.n = n;
